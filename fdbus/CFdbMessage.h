...
class CFdbMessage : public CFunctionJob
{
private:
...
public:
    CFdbMessage(FdbMsgCode_t code = FDB_INVALID_ID, EFdbQOS qos = FDB_QOS_DEFAULT);
     ...
     */
    const CFdbMsgMetadata *metadata() const;
    ...
    static bool kickDog(CBaseJob::Ptr &msg_ref, CBaseWorker *worker, tJobCallable fn);
    ...
    void doAutoReply(CBaseJob::Ptr &msg_ref, int32_t error_code, const char *description);
...
protected:
    ...
    virtual void onAsyncError(Ptr &ref, FdbMsgStatusCode code, const char *reason) {}
...
public:
    ...
    virtual CFdbMessage *clone(CFdbMessageHeader &head
                               , CBaseSession *session)
    {
        return 0;
    }
    ...
    bool buildHeader(CBaseSession *session);
    ...
    void doPublish(Ptr &ref);

    void setStatusMsg(int32_t error_code, const char *description = 0, EFdbMessageType type = FDB_MT_UNKNOWN);

    void sendStatus(CBaseSession *session, int32_t error_code, const char *description = 0);
    static void autoReply(CBaseSession *session, CBaseJob::Ptr &msg_ref, int32_t error_code, const char *description = 0);
    ...
    void update(CFdbMessageHeader &head, CFdbMsgPrefix &prefix);
    ...
    void encodeDebugInfo(CFdbMessageHeader &msg_hdr);
    void decodeDebugInfo(CFdbMessageHeader &msg_hdr);
    ...
    void callbackWithAutoReply(CBaseJob::Ptr &msg_ref, tJobCallable &func);
    bool subscribeNoQueue(int32_t timeout);
    ...
    friend class CFdbSetEvtDispatcher;
