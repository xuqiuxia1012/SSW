...
public:
    ...
    bool subscribe(CBaseNotification<T> *notification)
    {
        typename CBaseNotification<T>::Ptr ntf(notification);
        return subscribe(ntf);
    }
    ...
    void unsubscribe(typename CBaseNotification<T>::Ptr &notification)
    {
        ...
        std::lock_guard<std::mutex> _l(mMutex);
        auto it1 = mNtfTbl.find(event);
        if (it1 != mNtfTbl.end())
        {
            auto it2 = it1->second.find(notification);
     ...
     *  of the notifications will be called with data as paremeter.
     */
