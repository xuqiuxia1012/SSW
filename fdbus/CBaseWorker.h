...
{
public:
    // flag can be FDB_WORKER_ENABLE_FD_LOOP.
    CBaseWorker(const char* thread_name = "",
                uint32_t flag = FDB_WORKER_DEFAULT,
                uint32_t normal_queue_size = 0,
                uint32_t urgent_queue_size = 0);
    virtual ~CBaseWorker();
    ...
    /*
     * factory creating a work thread and start it
     *
     * @iparam flag - can be none or or-ed by FDB_WORKER_EXE_IN_PLACE
     *  and FDB_WORKER_ENABLE_FD_LOOP
     * @others are the same as CBaseWorker() 
     * @return the worker handle
     */
    static CBaseWorker *create(const char* thread_name = "",
                               uint32_t flag = FDB_WORKER_DEFAULT,
                               uint32_t normal_queue_size = 0,
                               uint32_t urgent_queue_size = 0);
    ...
    /*
     * send job to worker asynchronously; the job will be destroyed by the
     * worker automatically. The 'job' will be cleared and reference count
     * will be transferred transferred to job queue.
     *
     * @iparam job: the job to sent
     * @iparam urgent: true - job is inserted at the tail of urgent queue;
     *                 false - job is inserted at tail of normal queue
     ...
     * @return true - success; false - fail
     */
    bool sendAsyncSwap(CBaseJob::Ptr &job, bool urgent = false);
    bool sendAsyncSwapEndeavor(CBaseJob::Ptr &job, bool urgent);
    /*
     ...
     * @return true - success; false - fail
     */
    bool sendAsync(tJobCallable job_func, bool urgent = false, uint32_t job_flag = 0);
    bool sendAsyncEndeavor(tJobCallable job_func, bool urgent = false, uint32_t job_flag = 0);

    bool sendSync(tJobCallable job_func,
                 int32_t milliseconds = 0,
                 bool urgent = false,
                 uint32_t job_flag = 0);
    bool sendSyncEndeavor(tJobCallable job_func,
                          int32_t milliseconds = 0,
                          bool urgent = false,
                          uint32_t job_flag = 0);
...
private:
    ...
    public:
        ...
        bool enqueue(CBaseJob::Ptr &job, bool swap = false);
    ...
    bool runBeforeStarted(CBaseJob::Ptr &job);
    bool send(CBaseJob::Ptr &job, bool urgent, bool swap = false);
    bool send(CBaseJob::Ptr &job, int32_t milliseconds, bool urgent);
    void run();
